...
export const createCubeMatrix = (cubeSize: TCubeSize): TCubeMatrixObj => {
  const linearCubeMatrix = (cubeSize: TCubeSize) => [
    ...Array(cubeSize[0] * cubeSize[1] * cubeSize[2]).keys(),
  ];

  const linearCubeMatrixArr = linearCubeMatrix(cubeSize);

  const LRSize = [...Array(cubeSize[1] * cubeSize[2]).keys()]; // y * z
  const L = LRSize.map((i) => i);
  const R = LRSize.map(
    (i) => i + cubeSize[1] * cubeSize[2] * (cubeSize[0] - 1)
  );
  ...
  const allMs = Ms(noOfMs, cubeSize, linearCubeMatrixArr);
